...
const REAL_CODEX = '/home/mason/.npm-global/lib/node_modules/@openai/codex/bin/codex.js';
...
const args = shouldUseWeztermRunner
  ? [RUNNER, 'codex', REAL_CODEX, ...process.argv.slice(2)]
  : [REAL_CODEX, ...process.argv.slice(2)];
...
child.on('error', (error) => {
  console.error(`codex-wezterm: failed to start Codex: ${error.message}`);
  process.exit(127);
});
