...
then
    ...
    tmux send-keys -t $session:$window 'cd '$repo_location'/redacted/physician-portal/back-end' C-m
    tmux send-keys -t $session:$window 'npm run start' C-m
    tmux split-window -v
    tmux send-keys -t $session:$window 'cd ~/Desktop/proxy && node .' C-m
    tmux split-window -h
    tmux send-keys -t $session:$window 'cd '$repo_location'/redacted/scripts' C-m
    tmux select-pane -t 0
    tmux split-window -h
    tmux send-keys -t $session:$window 'cd '$repo_location'/redacted/patient-portal/back-end' C-m
    tmux send-keys -t $session:$window 'npm run start' C-m
