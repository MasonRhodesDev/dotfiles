...
# Theme toggle script for light/dark mode switching
...
generate_app_configs() {
    ...
    if /home/mason/scripts/generate-theme-configs.py "$WALLPAPER_PATH" "$mode"; then
        echo "Generated app configs successfully"
    else
...
reload_applications() {
    ...
    # Update Hyprland's environment variables (suppress "unknown request" messages)
    hyprctl setenv GTK_THEME "$GTK_THEME" 2>/dev/null || true
    hyprctl setenv QT_STYLE_OVERRIDE "$QT_STYLE_OVERRIDE" 2>/dev/null || true
    
    # Gently restart HyprPanel with better error handling
    if pgrep -f hyprpanel >/dev/null; then
        echo "Restarting HyprPanel..."
        pkill -TERM -f hyprpanel  # Use SIGTERM instead of SIGKILL
    ...
    # Start HyprPanel in background with error handling
    (uwsm app -- hyprpanel >/dev/null 2>&1) &
    sleep 2
    
    # Check if HyprPanel started successfully  
    if pgrep -f hyprpanel >/dev/null; then
        echo "✓ HyprPanel restarted successfully"
    else
        echo "⚠ HyprPanel failed to start - trying manual restart"
        # Try one more time without uwsm
        hyprpanel >/dev/null 2>&1 &
        sleep 1
        if pgrep -f hyprpanel >/dev/null; then
            echo "✓ HyprPanel started with fallback method"
        else
            echo "✗ HyprPanel restart failed - please start manually"
        fi
    fi
    ...
    # This signals many Electron apps to check for theme changes
    # Note: Cursor excluded as it may crash on SIGUSR1
    pkill -SIGUSR1 -f "code|discord|slack|spotify" 2>/dev/null || true
    
    # For Cursor, try a gentler approach - just update environment
    # It should pick up theme changes on next restart
    if pgrep -f cursor >/dev/null; then
        echo "Cursor detected - theme will apply on next restart"
    fi
    
    # Send theme change signal via xsettings for applications that support it
