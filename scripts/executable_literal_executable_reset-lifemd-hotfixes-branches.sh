...
# This script will reset the redacted hotfixes branches to the latest commit on the main branch via a force push.
# This script should be run from the root of the redacted repository.
...
# Check remote repository name is redacted-org/redacted
if [[ $(git remote get-url origin) != *"redacted-org/redacted"* ]]; then
    echo "Remote repository is not redacted-org/redacted"
    exit 1
fi
...
# ask user which hotfixes branch to reset [ hotfixes | redacted-hotfixes ]
echo "Which hotfixes branches do you want to reset?"
echo "1. hotfixes"
echo "2. redacted-hotfixes"
...
if [[ $hotfixes_branches == "1" ]]; then
    ...
    hotfixes_branch="redacted-hotfixes"
else
    echo "Exiting."
    exit 1
fi
